...
async def test_engine():
    ...
    async with engine.connect() as conn:
        ...
        if table_count == 0:
            raise RuntimeError(f"No tables found in test database! Expected tables from Base.metadata.")
...
@pytest.fixture
def hashed_password(password_service: BcryptPasswordService, valid_password: Password) -> HashedPassword:
